...
func _setup_shipyard(port_id: String) -> void:
	...
	if Fleet.ships.is_empty():
		...
		var berth := PanelContainer.new()
		berth.name = "Berth"
		berth.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		berth.custom_minimum_size = Vector2(0, 148)
		berth.add_theme_stylebox_override("panel", UiTheme.shore_door())
		var margin := MarginContainer.new()
		margin.add_theme_constant_override("margin_left", 14)
		margin.add_theme_constant_override("margin_top", 12)
		margin.add_theme_constant_override("margin_right", 14)
		margin.add_theme_constant_override("margin_bottom", 12)
		var berth_box := VBoxContainer.new()
		berth_box.add_theme_constant_override("separation", 6)
		margin.add_child(berth_box)
		berth.add_child(margin)
		choices_container.add_child(berth)
		...
		head.add_child(name_lbl)
		var aside := Label.new()
		aside.text = "帆　%s　甲　%s" % [_fit_rank(slv), _fit_rank(alv)]
		aside.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		UiTheme.style_footnote(aside)
		aside.add_theme_color_override("font_color", UiTheme.TEXT)
		head.add_child(aside)

		var stat := Label.new()
		stat.text = "水手 %d / %d　耐久 %d / %d　载 %d 料" % [
			Fleet.ship_crew(on),
		...
		if room > 0:
			...
			hire_chip.custom_minimum_size = Vector2(0, 34)
		...
		for j in Fleet.ships.size():
			if Fleet.ship_crew_room(j) > 0:
				...
				break
		if fleet_full:
			_slip_note(berth_box, "各船人手已满。")

		var others := DrydockBerth.other_hulls(Fleet.ships.size(), on)
		if others.size() > 0:
			var sw := HFlowContainer.new()
			sw.name = "YardSwitch"
			sw.add_theme_constant_override("h_separation", 8)
			sw.add_theme_constant_override("v_separation", 6)
			choices_container.add_child(sw)
			for idx in others:
				var other: Dictionary = Fleet.ships[idx]
				var swap := Button.new()
				swap.text = "换上　%s" % str(other.get("name", "船"))
				swap.custom_minimum_size = Vector2(0, 34)
				swap.pressed.connect(_on_berth_switch.bind(int(idx)))
				sw.add_child(swap)
				UiTheme.style_chip(swap)

	var dock := HBoxContainer.new()
	dock.name = "YardDock"
	dock.add_theme_constant_override("separation", 12)
	dock.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	choices_container.add_child(dock)
	_slip_host = dock
	...
	var sale_lbl := Label.new()
	sale_lbl.name = "YardSaleLabel"
	sale_lbl.text = "坞外待售"
	UiTheme.style_section_label(sale_lbl)
	choices_container.add_child(sale_lbl)
	...
	var sales := HFlowContainer.new()
	sales.name = "YardSales"
	sales.add_theme_constant_override("h_separation", 8)
	sales.add_theme_constant_override("v_separation", 6)
	choices_container.add_child(sales)
	for sid in for_sale:
		...
		var buy := Button.new()
		buy.text = "%s　购入　%d" % [str(offer.get("name", "船")), price]
		...
		if hist != "":
			buy.tooltip_text += "\n" + hist
		buy.pressed.connect(_on_buy_ship.bind(tid, price))
		sales.add_child(buy)
		UiTheme.style_chip(buy, true)

	var leave_row := HBoxContainer.new()
	leave_row.name = "YardLeave"
	...
	UiTheme.style_choice_button(leave)
	leave.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	choices_container.add_child(leave_row)
	choices_label.visible = false
